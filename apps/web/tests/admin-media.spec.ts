...
test('Media: trang list load + filter/search cập nhật URL', async ({ page, baseURL }) => {
  const resolvedBaseURL = baseURL ?? 'http://localhost:3000'
  const locale = 'vi'

  await gotoProtected(page, resolvedBaseURL, locale, `/${locale}/admin/media`)

  await expect(page.getByRole('heading', { name: 'Thư viện Media', level: 1 })).toBeVisible()
  ...
  await page.waitForURL(new RegExp(`\\?(.+&)?search=${searchValue}`), { timeout: 20_000 })
